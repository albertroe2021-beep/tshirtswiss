(()=>{
  const mount=document.getElementById('tss-header-en');
  if(!mount)return;

  const base='/wp-content/uploads/tshirtswiss-elementor-kit/assets/header/en/';
  const fragments=[
  ...
  Promise.all(fragments.map(fetchText))
    ...
    .catch(error=>{
      console.error('TShirtSwiss English header failed to load.',error);
    });
})();
